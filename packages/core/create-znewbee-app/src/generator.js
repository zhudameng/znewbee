...
class AppGenerator extends Generator {
  ...
  getContext() {
    ...
    switch (dbDialect) {
      case 'sqlite':
        ...
        envs.push(`DB_STORAGE=${env.DB_STORAGE || 'storage/db/znewbee.sqlite'}`);
        break;
      case 'mysql':
        if (!allDbDialect) {
          dependencies.push(`"mysql2": "^2.3.3"`);
  ...
  async downloadServerPackage() {
    ...
    console.log('Download: @znewbee/app-server');
    const serverPackageDir = resolve(this.cwd, 'packages/app/server');
    await downloadPackageFromNpm('@znewbee/app-server', serverPackageDir);
  ...
  async downloadClientPackage() {
    const { name } = this.context;
    console.log('Download: @znewbee/app-client');
    const clientPackageDir = resolve(this.cwd, 'packages/app/client');
    await downloadPackageFromNpm('@znewbee/app-client', clientPackageDir);
  ...
  async writing() {
    ...
    console.log(`Creating a new znewbee application at ${chalk.green(name)}`);
    ...
    console.log(chalk.green(`$ yarn znewbee install`));
