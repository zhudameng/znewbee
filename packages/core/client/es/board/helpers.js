...
function changeColumn(board, column, newColumn) {
  var changedColumns = replaceElementOfArray(board.columns)({
...
function addCard(board, inColumn, card) {
  ...
  var cards = addInArrayAtPosition(columnToAdd.cards, card, on === 'top' ? 0 : columnToAdd.cards.length);
  var columns = replaceElementOfArray(board.columns)({
...
function changeCard(board, cardId, newCard) {
  var changedCards = function changedCards(cards) {
    return replaceElementOfArray(cards)({
